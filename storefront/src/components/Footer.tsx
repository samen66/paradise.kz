...
export async function Footer({ settings, categories }: { settings: Settings | null; categories: Category[] }) {
  ...
  const hasContacts = contacts?.phone || contacts?.email || contacts?.address || contacts?.whatsapp_url || contacts?.instagram_url;
            ...
            <ul className="space-y-4 text-sm">
              {(settings?.stores ?? []).map((store) => (
                <li key={store.id} className="text-muted leading-relaxed">
                  <span className="font-medium text-ink block">{tValue(store.name, locale)}</span>
