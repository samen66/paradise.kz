  ...
  if (loadError === 'not_found') {
    return (
      <div className="space-y-3">
        ...
        <Link href={warehouseHref.documents('write_offs')} className={buttonLink}>← К списаниям</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        back={warehouseHref.documents('write_offs')}
        actions={
          isDraft ? (
            <>
              ...
              <ConfirmButton
                ...
                onConfirm={async () => {
                  ...
                  router.push(warehouseHref.documents('write_offs'));
                }}
              >
                Удалить черновик
              </ConfirmButton>
      ...
      {!isDraft && (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">
          ...
          <Link href={`${warehouseHref.movements}?document=write_off:${writeOff.id}`} className={buttonLink}>Движения по этому списанию</Link>
        </div>
      )}
