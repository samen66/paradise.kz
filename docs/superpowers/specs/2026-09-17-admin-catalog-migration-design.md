...
- Контракт API товара меняется: `show` отдаёт `images` вместо `media`;
  `store`/`update` больше не принимают `images` (картинки — через
  `/admin/products/{id}/media`); `destroy` может вернуть 422, если на товар
  есть движения по складу или строки приёмки. `admin/` — единственный
  потребитель, обновляется в том же PR.
- Загрузка фото требует `upload_max_filesize` ≥ 12M и `post_max_size` ≥ 16M в
  PHP (сейчас 2M/8M по умолчанию) — правится в `Dockerfile` и
  `docker/php/Dockerfile.prod` (`zz-app.ini`), нужен пересборка образа.
