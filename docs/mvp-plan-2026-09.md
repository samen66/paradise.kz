...
### B2 — в `admin/` нельзя создать товар — P0

`admin/src/app/products/` содержит только `page.tsx` (список) и `[id]/page.tsx` (редактирование).
Страницы создания нет, хотя `POST /api/admin/products` реализован.

**Файлы:** новый `admin/src/app/products/new/page.tsx`, кнопка «Создать товар» в `admin/src/app/products/page.tsx`.

**Суть:** переиспользовать форму из `[id]/page.tsx`, вынеся её в `admin/src/components/ProductForm.tsx`.

**Приёмка:** товар создаётся из `admin/` и виден в публичном каталоге (при активной категории + цене + остатке).
